<template>
  <div class="flex flex-row justify-between hover:bg-blue-200">
    <div class="flex flex-row py-4">
      <div class="w-28 ml-8">{{ loan.startDate }}</div>
      <div class="w-28">{{ loan.endDate }}</div>
      <div class="w-28">{{ loan.userFirstName }}</div>
      <div class="w-28">{{ loan.userLastName }}</div>
      <div class="w-24">{{ loan.copyNumber }}</div>
