<template>
    <main id="content" role="main" class="w-full max-w-md mx-auto p-6">
        ...
        <div class="mt-7 bg-neutral-50  rounded-xl shadow-lg ">
            <div class="p-4 sm:p-7">
                <div class="text-center">
                    <!-- removed: dark:text-white-->
                    <h1 class="block text-2xl font-bold text-gray-800 ">Wachtwoord vergeten?</h1>
                    ...
                    <form @submit.prevent="resetPassword">
                        <div class="grid gap-y-4">
                            <div>
                                ...
                                <p class="hidden text-xs text-red-600 mt-2" id="email-error">Voer een geldige email adres in</p>
    ...
    methods: {
        resetPassword() {            
            if (this.user.newPassword == this.newPasswordCheck) {
                axios.put('http://localhost:8080/user/reset-password', this.user)
                    .then(response => {
                        if (response.data) {
